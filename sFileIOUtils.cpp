...
bool sFileIOUtils::CreateDirectory(const std::string& path)    //throw(runtime_error)
{
  ...
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils>> Error creating dir: '%s'\n", path.c_str());
...
//     << path << endl;
  ...
  {
//     RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils>> Directory: '%s' successfully created.\n", path.c_str());
    return true;
  }
}
...
std::string sFileIOUtils::CreateTempfile()
{
  ...
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::CreateTempfile() >> Error creating temp file!\n");
    exit(EXIT_FAILURE);
  }
...
void sFileIOUtils::CompressFile(const std::string& input_filename, std::string& output_filename)
{
  ...
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils>> Execution of command '%s' failed!\n", cmd.str().c_str());
    exit(EXIT_FAILURE);
  }
} // CompressFile()
...
bool sFileIOUtils::ShellExpandFilename(std::string &result, const std::string &file_name)
{
  ...
  if (error)
  {
    ...
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::%s >> Could not expand '%s': %s!\n", __FUNCTION__, file_name.c_str(), error_msg);
    return false;
  }
...
{
  //RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::GetHostName() >>> started\n");
  std::string cmd = fqdn ? "hostname -f" : "hostname";
  FILE * pipe = popen(cmd.c_str(), "r");
  char buf[ 1024 ];
  if (fgets(buf, sizeof(buf), pipe) == 0)
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::GetHostName(bool fqdn) >> Error querying host name!\n");
    exit(EXIT_FAILURE);
  }
  pclose(pipe);
  std::string name(buf);
  ...
  //boost::trim(name);
  //RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::GetHostName() >>> finished with name <%s>\n", name.c_str());
  return name;
} // GetHostName()
...
struct ::in_addr sFileIOUtils::HostToIpViaGetHostByName(const std::string& name)
{
  ...
  if (host_ent)
  {
    ...
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "ip address of host <%s> = <%s>\n", name.c_str(), inet_ntoa(address));
  }
  else
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::GetIpAddressOfHost() >>> could not get ip address of host <%s>\n", name.c_str());
  }
  return address;
...
{
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaNslookup() >>> started with host <%s>\n", name.c_str());
  struct in_addr address;
  address.s_addr = 0;

  std::stringstream command;
  command << "nslookup \"" << name << "\"";
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "command = <%s>\n", command.str().c_str());
  ...
  pclose(pipe);
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaNslookup() >>> host = <%s> , result = <%s> \n", name.c_str(), result.str().c_str());
  ...
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "Could not find token <%s> in nslookup result ... returning <%s> \n", search_token.c_str(), inet_ntoa(address));
    return address;
  }
  result_str.erase(0, pos + search_token.length());
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "pos = %zd\n", pos);
  ...
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "found_name <%s> , name <%s> \n", found_name.c_str(), name.c_str());

  if (found_name != name)
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "nslookup failed .... returning <%s> \n", inet_ntoa(address));
    return address;
  }
...
{
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaHost() >>> started with host <%s>\n", name.c_str());
  struct in_addr address;
  address.s_addr = 0;
  ...
  pclose(pipe);
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaHost() >>> host = <%s> , result = <%s> \n", name.c_str(), result.str().c_str());
...
{
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaHostsFile() >>> started with host <%s>\n", name.c_str());
  struct in_addr address;
  address.s_addr = 0;
    ...
    if (line_str.length() > 0 && line_str.find("#") == std::string::npos)
    {
      ...
      if (find(tokens.begin(), tokens.end(), name) != tokens.end())
      {
        for (std::vector<std::string>::const_iterator iter = tokens.begin(); iter != tokens.end(); ++iter)
        {
          ...
          {
            ip_address = *iter;
            RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIpViaHostsFile() >>> got ip <%s> of host <%s> from hosts file\n", ip_address.c_str(), name.c_str());
            found = true;
            break;
          }
        }
...
{
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIp() >>> started with host <%s>\n", name.c_str());
  ...
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIp() >>> got ip <%s> of host <%s> from hosts_file\n", inet_ntoa(address), name.c_str());
  }
  else if ((address = sFileIOUtils::HostToIpViaHost(name)).s_addr != 0)
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIp() >>> got ip <%s> of host <%s> via <host -t A>\n", inet_ntoa(address), name.c_str());
  }
  else if ((address = sFileIOUtils::HostToIpViaNslookup(name)).s_addr != 0)
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::HostToIp() >>> got ip <%s> of host <%s> via <nslookup>\n", inet_ntoa(address), name.c_str());
  }
  else
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::HostToIp() >>> could not get ip of host <%s>\n", name.c_str());
  }
...
int sFileIOUtils::RSyncFile(const std::string& source_host_name, const std::string& source_host_ip_address, const std::string& source_directory, const std::string& source_file_name, const std::string& target_directory, const std::string &optional_rsync_flags)
{
  ...
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::RSyncFile() >>> executing <%s> ...\n", rsync_command.str().c_str());
  int ret = system(rsync_command.str().c_str());
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::RSyncFile() >>> ... done.\n");

  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::RSyncFile() >>> finished with result %d\n", ret);
  return ret;
} // RSyncFile()
...
int sFileIOUtils::RSyncFiles(const std::string& source_host_name, const std::string& source_host_ip_address, const std::string& source_directory, const std::vector<std::string>& source_file_names, const std::string& target_directory, const std::string &optional_rsync_flags)
{
  ...
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::RSyncFiles() >>> executing <%s> ...\n", rsync_command.str().c_str());
  int ret = system(rsync_command.str().c_str());
  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_USER, "sFileIOUtils::RSyncFiles() >>> ... done.\n");

  RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::RSyncFiles() >>> finished with result %d\n", ret);
  return ret;
} // RSyncFiles()
...
bool sFileIOUtils::CheckAndGetFile(const std::string &file_name, std::string &full_local_file_name, const std::string& resource_repository, const std::string& resource_server, const std::string& local_resource_directory, const std::string& server_resource_directory, bool use_cache)
{
  ...
  else
  {
    ...
    {
      RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::%s >> Could not expand local resource directory!\n", __FUNCTION__);
      return false;
    }
    ...
    else
    {
      ...
      if (server != "")
      {
        ...
        else
        {
          ...
          {
            RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::constructor >>> zero ip address ... clearing server\n");
            server = "";
          }
          else
          {
      ...
      if (server != "")   //&& server != local_host)
      {
        ...
        {
          RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::CheckAndGetFile() >>> could not rsync file <%s> from server <%s> at <%s> ... skipping\n", file_name.c_str(), server.c_str(), (server_resource_directory + resource_repository).c_str());
          return false;
        }
        full_local_file_name = (expanded_local_resource_directory + resource_repository + file_name);
      }
      ...
      {
        RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::CheckAndGetFile() >>> could neither load <%s> locally nor from resource server ... skipping\n", file_name.c_str());
        return false;
      }
    }
  }
  ...
  if (!try_3)
  {
    RRLIB_LOG_MESSAGE(rrlib::logging::eLL_ERROR, "sFileIOUtils::CheckAndGetFile() >>> final check failed: could not load <%s> ... skipping\n", full_local_file_name.c_str());
    return false;
  }
  else
  {
